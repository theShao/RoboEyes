...
public class BlobFinder
{
	...
	static class Blob implements Comparable<Blob>
	{
		...
		public Blob(int xMin, int xMax, int yMin, int yMax, int mass)
		{
			...
			this.yMin = yMin;
			this.yMax = yMax;
			this.cogX = (xMin + xMax)/2;
			this.cogY = (yMin + yMax)/2;
			this.mass = mass;
		}
		...
		public String toString()
		{
		  ...
		  public int compareTo(Blob b) throws ClassCastException {
			    return this.mass - ((Blob)b).mass;
			  }
	}
	...
	public BlobFinder(int width, int height)
	{
		...
		int tableSize = width * height / 10; //modded because we're not interesed in tiny blobs
	...
	public List<Blob> detectBlobs(byte[] srcData, byte[] dstData, int minBlobMass, int maxBlobMass, byte matchVal, List<Blob> blobList)
	{
		...
		for (int i=label-1 ; i>0 ; i--)
		{
			...
			else
			{
				...
				{
					Blob blob = new Blob(xMinTable[i], xMaxTable[i], height - yMinTable[i], height - yMaxTable[i], massTable[i]);
					blobList.add(blob);
